...
class Seller:
    ...
    def get_review_score(self):
        ...
        # Ensure numeric scores (defensive)
        df["review_score"] = pd.to_numeric(df["review_score"], errors="coerce")

        # cost mapping: 1->100, 2->50, 3->40, else 0
    ...
    def get_training_data(self):
        ...
        if review_df is not None:
            ...
            training_set["share_of_one_stars"] = np.nan
            training_set["share_of_five_stars"] = np.nan
            training_set["review_score"] = np.nan
            training_set["cost_of_reviews"] = 0

        training_set["cost_of_reviews"] = training_set["cost_of_reviews"].fillna(0)

        # revenues = subscription fees + sales fees
        # sales fees = 10% of sales (freight excluded)
        months = training_set["months_on_olist"].fillna(0)
        billed_months = np.maximum(1, np.ceil(months))
        training_set["revenues"] = 0.10 * training_set["sales"] + 80 * billed_months
        ...
        training_set = training_set[[c for c in cols if c in training_set.columns]]
